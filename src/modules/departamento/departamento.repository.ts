...
export class DepartamentoRepository extends AbstractRepository<Departamento> {
  async store(body: any): Promise<Departamento> {
    const { nombre } = body
  ...
  async findOne(departamento_id: string): Promise<Departamento> {
  ...
  async update(departamento_id: string, body: any): Promise<Departamento> {
    const { nombre } = body
  ...
  async delete(departamento_id: string): Promise<Departamento> {
    const departamento = await this.repository.findOneOrFail({
